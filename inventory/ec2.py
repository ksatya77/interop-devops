...
class Ec2Inventory(object):
    ...
    def __init__(self):
        ...
        # Set environment
        self.env = os.environ.get('MODUSBOX_ENV')
    ...
    def read_settings(self):
        ...
        if config.has_option('ec2',
                             'all_elasticache_replication_groups') and self.elasticache_enabled:
            self.all_elasticache_replication_groups = config.getboolean('ec2',
        ...
        cache_id = self.boto_profile or os.environ.get('AWS_ACCESS_KEY_ID',
        ...
        try:
            ...
            if self.ec2_instance_filters:
                if self.stack_filters:
                    ...
                    for filter_key, filter_values in self.ec2_instance_filters.items():
                        reservations.extend(
                            conn.get_all_instances(filters={filter_key: filter_values}))
            else:
            ...
            for i in range(0, len(instance_ids), max_filter_value):
                tags.extend(conn.get_all_tags(filters={'resource-type': 'instance',
                                                       'resource-id': instance_ids[
                                                                      i:i + max_filter_value]}))
        ...
        for c in clusters:
            ...
            try:
                ...
                tags = client.list_tags_for_resource(
                    ResourceName='arn:aws:rds:' + region + ':' + account_id + ':cluster:' + c[
                        'DBClusterIdentifier'])
                c['Tags'] = tags['TagList']
    ...
    def get_elasticache_clusters_by_region(self, region):
        ...
        try:
            ...
            clusters = response['DescribeCacheClustersResponse']['DescribeCacheClustersResult'][
                'CacheClusters']
    ...
    def get_auth_error_message(self):
        ...
        if len(boto_config_found) > 0:
            errors.append(
                " - Boto configs found at '%s', but the credentials contained may not be correct" % ', '.join(
                    boto_config_found))
        else:
            errors.append(
    ...
    def add_instance(self, instance, region):
        ...
        # Inventory: Group by environment and role
        role = instance.tags.get('role')
        for k, v in instance.tags.items():
            if k == 'environment' and v == self.env and role:
                self.push(self.inventory, role, hostname)
    ...
    def add_elasticache_cluster(self, cluster, region):
        ...
        if self.group_by_elasticache_parameter_group:
            self.push(self.inventory, self.to_safe(
                "elasticache_parameter_group_" + cluster['CacheParameterGroup'][
                    'CacheParameterGroupName']), dest)
            if self.nested_groups:
        ...
        if self.group_by_elasticache_replication_group and 'ReplicationGroupId' in cluster and \
    ...
    def add_elasticache_node(self, node, cluster, region):
        ...
        if self.group_by_elasticache_cluster:
            self.push(self.inventory,
                      self.to_safe("elasticache_cluster_" + cluster['CacheClusterId']), dest)
            ...
            if key == 'ec2_node_groups' and value:
                ...
                for node in value[0]['NodeGroupMembers']:
                    if node['CurrentRole'] == 'primary':
                        ...
                        host_info['ec2_replica_cluster_address_' + str(replica_count)] = \
                        node['ReadEndpoint']['Address']
                        host_info['ec2_replica_cluster_port_' + str(replica_count)] = \
                        node['ReadEndpoint']['Port']
                        host_info['ec2_replica_cluster_id_' + str(replica_count)] = node[
                            'CacheClusterId']
                        replica_count += 1
